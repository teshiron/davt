...
AVT.diffDisplay = function(title, editor, timestamp, summary, matches, content, revid, isNewPage, isKnownVandal){ //function to generate and append the HTML to display a matching diff
    ...
    //add a link to open the diff in a new page - this allows use of tools like TW for rollback if the edit is bad but doesn't fall within the types this tool covers
    newHTML += '[<a href="https://en.wikipedia.org/w/index.php?oldid=' + revid + '&diff=prev" target="_blank">open diff</a>] ';
    ...
    rollbackLine += rollbackfrag + "'spam')\">spam links</a>] " + rollbackfrag + "'advert')\">advert/promo</a>] ";
...
AVT.rollback = function(editor, revid, divNumber, warnType) { //this function uses the API to roll back, which still requires the rollback right
    ...
    switch (warnType) {
        ...
        case "defam":
            ...
            break;
        case "spam":
            rollbackSummary += "[[WP:SPAM|spam link(s)]] ";
            warningPriority = true;
            break;
        case "advert":
            rollbackSummary += "promotional material ";
            warningPriority = true;
            break;
        default:
